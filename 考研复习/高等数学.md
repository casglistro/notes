...
$$
\dfrac{\partial f}{\partial l}|_{(x_{0},y_{0},z_{0})}=\lim_{ t \to 0^+ } \dfrac{f(x_{0}+t\cos \alpha,y_{0}+t\cos \beta,z_{0}+\cos \gamma)-f(x_{0},y_{0},z_{0})}{t} 
$$
注意这里趋于的是 $0^+$。因此有 $|t|=t$。
...
$$
curl \ F(1,1,0) = (\dfrac{\partial R}{\partial y}-\dfrac{\partial Q}{\partial z})i=(--y)i=i
$$
